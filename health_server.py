...
class HealthStatus ():
  ...
  def initialize (self, args):
    ...
    try:
      ...
      self.health_obj.initialize (config, args.addr, args.port,args.demux_token)
  ...
  ########################################
  def gen_response_msg (self,code,contents):
    ...
    resp_msg = ResponseMessage (code,contents)
  ...
  def driver (self):
    try:
      ...
      while True:
        ...
        if str(request.msg_type) == "MessageTypes.HEALTH":
          resp = self.gen_response_msg (1, "You Are Healthy!")
        else:
          resp = self.gen_response_msg (2, "Bad Request!")
...
def parseCmdLineArgs ():
  ...
  parser.add_argument ("-t", "--demux_token", default="health_server", help="Our identity used as a demultiplexing token (default: router)")
