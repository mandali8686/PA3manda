...
from applnlayer.ApplnMessageTypes import MilkType
from applnlayer.ApplnMessageTypes import BreadType
from applnlayer.ApplnMessageTypes import MeatType
import router as routerObj
...
class Refrigerator ():
  ...
  def initialize (self, args):
    ...
    try:
      ...
      self.router_obj= ApplnProtoObj (False)

      # initialize the custom application objects
      self.groc_obj.initialize (config, args.groc_ip, args.groc_port,args.demux_token)
      self.health_obj.initialize (config, args.status_ip, args.status_port,args.demux_token)
      self.router_obj.initialize (config, args.status_ip, args.status_port,args.demux_token)
...
def parseCmdLineArgs ():
  ...
  parser.add_argument ("-g", "--groc_ip", default="10.0.0.2", help="IP Address of Grocery server to connect to (default: localhost i.e., 127.0.0.1)")
  parser.add_argument ("-p", "--groc_port", type=int, default=4444, help="Port that grocery server is listening on (default: 4444)")
  parser.add_argument ("-s", "--status_ip", default="10.0.0.2", help="IP Address of Health Status server to connect to (default: localhost i.e., 127.0.0.1)")
  parser.add_argument ("-q", "--status_port", type=int, default=4444, help="Port that Healt Status server is listening on (default: 4444)")
